...
def medias(tabMeteo):
    """Calcula a temperatura média de cada dia."""
    return [(data, (temp_min + temp_max) / 2) for (data, temp_min, temp_max, _) in tabMeteo]

def guardaTabMeteo(t, fnome):
    """Guarda a tabela meteorológica num ficheiro de texto."""
    with open(fnome, 'w') as f:
        for registo in t:
            f.write(f"{registo}\n")

def carregaTabMeteo(fnome):
    """Carrega a tabela meteorológica de um ficheiro de texto."""
    res = []
...
def minMin(tabMeteo):
    """Calcula a temperatura mínima mais baixa registada na tabela."""
    return min(temp_min for (_, temp_min, _, _) in tabMeteo)

def amplTerm(tabMeteo):
    """Calcula a amplitude térmica de cada dia."""
    return [(data, temp_max - temp_min) for (data, temp_min, temp_max, _) in tabMeteo]

def maxChuva(tabMeteo):
    """Calcula o dia em que a precipitação registada teve o seu valor máximo."""
...
def diasChuvosos(tabMeteo, p):
    """Retorna os dias em que a precipitação foi superior a p."""
    return [(data, precipitacao) for (data, _, _, precipitacao) in tabMeteo if precipitacao > p]

def maxPeriodoCalor(tabMeteo, p):
    """Retorna o maior número consecutivo de dias com precipitação abaixo de p."""
    max_consecutivo = 0
    atual_consecutivo = 0
...
def grafTabMeteo(t):
    """Desenha os gráficos da temperatura mínima, máxima e de pluviosidade."""
...
def menu():
    ...
    while continuar:
        ...
        if opcao == '1':
            ...
            data = tuple(map(int, input("Informe a data (ano, mês, dia): ").split()))
            temp_min = float(input("Informe a temperatura mínima: "))
            temp_max = float(input("Informe a temperatura máxima: "))
            precipitacao = float(input("Informe a precipitação: "))
